...
// Main batch processor - only processes lessons WITHOUT content
async function main() {
  const args = process.argv.slice(2);
  const maxCount = parseInt(args[0]) || 10; // Max lessons to process
    ...
    for (const subject of year.Subject) {
      for (const unit of subject.Unit) {
        for (const lesson of unit.Lesson) {
          // Check if lesson has NO content or very minimal content
  ...
  console.log(`📝 Processing up to ${maxCount} lessons...\n`);

  let processed = 0;
  let failed = 0;
  const toProcess = lessonsWithoutContent.slice(0, maxCount);
  ...
  console.log(`   📚 Lessons remaining without content: ${lessonsWithoutContent.length - processed}\n`);
}
